...
tf.app.flags.DEFINE_integer('max_train_epoch', MAX_TRAIN_EPOCH,
                            '')
tf.app.flags.DEFINE_integer('save_every_n_steps', int(math.ceil(float(SAVE_EVERY_N_EPOCH)*len(train_labels)/BATCH_SIZE)),
                            '')
...
tf.app.flags.DEFINE_integer('max_number_of_steps', int( math.ceil(float(MAX_TRAIN_EPOCH)*len(train_labels)/BATCH_SIZE)),
  ...
  with slim.arg_scope([slim.batch_norm, slim.dropout], is_training=is_training):
    with slim.arg_scope(inception_v3_arg_scope(weight_decay=weight_decay)):
      with tf.variable_scope("loc") as scope:
        ...
        loc_net = slim.conv2d(loc_net, 128, loc_net.get_shape()[1:3], padding='VALID', activation_fn=tf.nn.tanh, scope='Loc_fc1')
        loc_net = slim.flatten(loc_net)
        iv = 4.
        initial = np.array([iv, 0, 0, 0, iv, 0]*NUM_STN, dtype=np.float32)
        b_fc_loc = tf.get_variable("Loc_fc_b", shape = [6*NUM_STN], initializer=init_ops.constant_initializer(initial), dtype=dtypes.float32)
        W_fc_loc = tf.get_variable("Loc_fc_W", shape = [128, 6*NUM_STN], initializer=init_ops.constant_initializer(np.zeros( (128,6*NUM_STN) )), dtype=dtypes.float32)
        theta = tf.nn.tanh(tf.matmul(loc_net, W_fc_loc) + b_fc_loc)
      _finals = []
      for i in xrange(NUM_STN):
        scope_name = "stn%d"%i
        with tf.variable_scope(scope_name) as scope1:
          _theta = tf.slice(theta, [0, 6*i], [-1, 6*(i+1)])
...
def do_training(train_op, init_fn=None, summary_op=None, lr = None):
  ...
  with graph.as_default():
    ...
    prefix = "loc/net"
    lp = len(prefix)
    vdic = {"InceptionV2"+v.op.name[lp:] : v for v in tf.trainable_variables() if v.name.startswith(prefix) and v.name.find("Logits/")<0}
